...
/* @brief Generate the prior boxes of designated sizes and aspect ratios across all dimensions @f$ (H \times W) @f$.
 * Intended for use with MultiBox detection method to generate prior (template). NOTE: does not implement Backwards operation. */
template <typename Dtype>
class PriorBoxLayer : public Layer<Dtype> {
 public:
  /* @param param provides PriorBoxParameter prior_box_param, with PriorBoxLayer options:
 ...
 protected:
  ...
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, 
                            const vector<Blob<Dtype>*>& bottom) { return; }
