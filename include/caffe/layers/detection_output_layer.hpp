...
/* @brief Generate the detection output based on location and confidence predictions by doing non maximum suppression.
 * Intended for use with MultiBox detection method. NOTE: does not implement Backwards operation. */
template <typename Dtype>
class DetectionOutputLayer : public Layer<Dtype> {
 public:
  explicit DetectionOutputLayer(const LayerParameter& param) : Layer<Dtype>(param) {}
 ...
 protected:
   ...
   *      N is the number of detections after nms, and each row is: [image_id, label, confidence, xmin, ymin, xmax, ymax] */
