 ...
 * This layer should be preferred over separate SoftmaxLayer + MultinomialLogisticLossLayer
 ...
 *      @f$ [-\infty, +\infty] @f$ indicating the predicted score for each of the @f$ K = CHW @f$ classes.
 *      This layer maps these scores to a probability distribution over classes using the softmax function
 ...
 *      the computed cross-entropy classification loss: @f$ E = \frac{-1}{N} \sum\limits_{n=1}^N \log(\hat{p}_{n,l_n})
...
class SoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:
    ...
    *    If true, the loss is normalized by the number of (nonignored) labels present; 
    *    otherwise the loss is simply summed over spatial locations. */
  explicit SoftmaxWithLossLayer(const LayerParameter& param) : LossLayer<Dtype>(param) {}
