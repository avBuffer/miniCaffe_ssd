...
/* @brief Applies common transformations to the input data, such as scaling, mirroring, substracting the image mean... */
...
class DataTransformer {
 public:
  ...
  /* @brief Applies the transformation defined in the data layer's transform_param block to the data.
   * @param datum: Datum containing the data to be transformed.
   * @param transformed_blob: This is destination blob. It can be part of top blob's data if set_cpu_data() is used. 
   *                          See data_layer.cpp for an example. */
  void Transform(const Datum& datum, Blob<Dtype>* transformed_blob);

  /* @brief Applies the transformation defined in the data layer's transform_param block to a vector of Datum.
   * @param datum_vector: A vector of Datum containing the data to be transformed.
   * @param transformed_blob: This is destination blob. It can be part of top blob's data if set_cpu_data() is used. 
   *                          See memory_layer.cpp for an example. */
  ...
  /* @brief Transform the annotation according to the transformation applied to the datum.
   * @param anno_datum
   *    AnnotatedDatum containing the data and annotation to be transformed.
   * @param do_resize
  ...
  void TransformAnnotation(const AnnotatedDatum& anno_datum, const bool do_resize, const NormalizedBBox& crop_bbox, 
                           const bool do_mirror, RepeatedPtrField<AnnotationGroup>* transformed_anno_group_all);
  ...
  void CropImage(const AnnotatedDatum& anno_datum, const NormalizedBBox& bbox, AnnotatedDatum* cropped_anno_datum);
  /* @brief Expand the datum. */
  void ExpandImage(const Datum& datum, const float expand_ratio, NormalizedBBox* expand_bbox, Datum* expanded_datum);
...
#ifdef USE_OPENCV
  /* @brief Applies the transformation defined in the data layer's transform_param block to a vector of Mat.
   * @param mat_vector: A vector of Mat containing the data to be transformed.
   * @param transformed_blob: This is destination blob. It can be part of top blob's data if set_cpu_data() is used. 
   *                          See memory_layer.cpp for an example. */
  void Transform(const vector<cv::Mat> & mat_vector, Blob<Dtype>* transformed_blob);

  /* @brief Applies the transformation defined in the data layer's transform_param block to a cv::Mat
   * @param cv_img: cv::Mat containing the data to be transformed.
   * @param transformed_blob: This is destination blob. It can be part of top blob's data if set_cpu_data() is used.
   *                          See image_data_layer.cpp for an example. */
  ...
  /* @brief Applies the same transformation defined in the data layer's transform_param block to all the num images in a input_blob.
   * @param input_blob: A Blob containing the data to be transformed. It applies the same transformation to all the num images in the blob.
   * @param transformed_blob: This is destination blob, it will contain as many images as the input blob. It can be part of top blob's data. */
 ...
 protected:
   ...
   * @param n: The upperbound (exclusive) value of the random number.
   * @return: A uniformly random integer value from ({0, 1, ..., n-1}). */
