...
class PascalMultilabelDataLayerSync(caffe.Layer):
    """This is a simple synchronous datalayer for training a multilabel model on PASCAL."""
    def setup(self, bottom, top):
        ...
        # === reshape tops ===
        # since we use a fixed input image size, we can shape the data layer once. 
        # Else, we'd have to do it in the reshape call.
...
class BatchLoader(object):
    """This class abstracts away the loading of images. Images can either be loaded singly, or in a batch. 
    The latter is used for the asyncronous data layer to preload batches while other processing is performed."""
    def __init__(self, params, result):
        ...
        self.indexlist = [line.rstrip('\n') for line in open(osp.join(self.pascal_root, 'ImageSets/Main', list_file))]
    ...
    def load_next_image(self):
        ...
        for label in anns['gt_classes']:
            ...
            # The "-1" is b/c we are not interested in the background class.
...
def load_pascal_annotation(index, pascal_root):
    """This code is borrowed from Ross Girshick's FAST-RCNN code (https://github.com/rbgirshick/fast-rcnn).
    It parses the PASCAL .xml metadata files. See publication for further details: (http://arxiv.org/abs/1504.08083)."""
    ...
    for ix, obj in enumerate(objs):
        ...
        cls = class_to_ind[str(get_data_from_tag(obj, "name")).lower().strip()]
...
def print_info(name, params):
    ...
    print "{} initialized for split: {}, with bs: {}, im_shape: {}.".format(name, params['split'], params['batch_size'], params['im_shape'])
