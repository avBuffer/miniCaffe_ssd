...
def array_to_blobproto(arr, diff=None):
    """Converts a N-dimensional array to blob proto. If diff is given, also convert the diff.
    You need to make sure that arr and diff have the same shape, and this function does not do sanity check."""
...
def array_to_datum(arr, label=None):
    ...
    the output data will be encoded as a string. Otherwise, the output data will be stored in float format."""
    if arr.ndim != 3:
...
def datum_to_array(datum):
    ...
    if len(datum.data):
        return np.fromstring(datum.data, dtype=np.uint8).reshape(datum.channels, datum.height, datum.width)
    else:
        return np.array(datum.float_data).astype(float).reshape(datum.channels, datum.height, datum.width)
...
class Transformer:
    ...
    def __check_input(self, in_):
        if in_ not in self.inputs:
            raise Exception('{} is not one of the net inputs: {}'.format(in_, self.inputs))
    ...
    def set_transpose(self, in_, order):
        ...
        if len(order) != len(self.inputs[in_]) - 1:
            raise Exception('Transpose order needs to have the same number of dimensions as the input.')
    ...
    def set_channel_swap(self, in_, order):
        ...
        if len(order) != self.inputs[in_][1]:
            raise Exception('Channel swap needs to have the same number of dimensions as the input channels.')
    ...
    def set_raw_scale(self, in_, scale):
        ...
        like CaffeNet and AlexNet represent images in [0, 255] 
        so the raw_scalen of these models must be 255.
        Parameters
    ...
    def set_input_scale(self, in_, scale):
        ...
        N.B. input_scale is done AFTER mean subtraction and other preprocessing while raw_scale is done BEFORE.
        Parameters
            in_ : which input to assign this scale factor
            scale : scale coefficient"""
        self.__check_input(in_)
...
def load_image(filename, color=True):
    ...
    Parameters
        filename : string
        color : boolean, flag for color format. True (default) loads as RGB while False
                loads as intensity (if image is already grayscale).
    Returns
        image : an image with type np.float32 in range [0, 1] of size (H x W x 3) in RGB or of size (H x W x 1) in grayscale."""
...
def oversample(images, crop_dims):
    ...
    # Extract crops
    crops = np.empty((10 * len(images), crop_dims[0], crop_dims[1], im_shape[-1]), dtype=np.float32)
    ix = 0
    for im in images:
