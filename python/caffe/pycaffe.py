...
# We directly update methods from Net here (rather than using composition or inheritance)
# so that nets created by caffe (e.g., by SGDSolver) will automatically have the improved interface.
...
def _Net_blob_loss_weights(self):
    ...
    if not hasattr(self, '_blobs_loss_weights_dict'):
        self._blob_loss_weights_dict = OrderedDict(zip(self._blob_names, self._blob_loss_weights))
    return self._blob_loss_weights_dict


@property
def _Net_params(self):
    """An OrderedDict (bottom to top, i.e., input to output) of network parameters indexed by name;
    each is a list of multiple blobs (e.g., weights and biases)"""
    if not hasattr(self, '_params_dict'):
        self._params_dict = OrderedDict([(name, lr.blobs) for name, lr in zip(self._layer_names, self.layers) if len(lr.blobs) > 0])
...
def _Net_forward(self, blobs=None, start=None, end=None, **kwargs):
    ...
    Parameters
        ...
        kwargs : Keys are input blob names and values are blob ndarrays.
                 For formatting inputs for Caffe, see Net.preprocess(). If None, input is taken from data layers.
    ...
    if kwargs:
        ...
        # Set input according to defined shapes and make arrays single and C-contiguous as Caffe expects.
...
def _Net_backward(self, diffs=None, start=None, end=None, **kwargs):
    ...
    if kwargs:
        ...
        # Set top diffs according to defined shapes and make arrays single and C-contiguous as Caffe expects.
...
def _Net_forward_backward_all(self, blobs=None, diffs=None, **kwargs):
    ...
    forward_batches = self._batch({in_: kwargs[in_] for in_ in self.inputs if in_ in kwargs})
    backward_batches = self._batch({out: kwargs[out] for out in self.outputs if out in kwargs})
...
def _Net_set_input_arrays(self, data, labels):
    ...
    if labels.ndim == 1:
        labels = np.ascontiguousarray(labels[:, np.newaxis, np.newaxis, np.newaxis])
...
def _Net_batch(self, blobs):
    ...
    if remainder > 0:
        padded_batch = {}
        for name in blobs:
            padding = np.zeros((batch_size - remainder,) + blobs[name].shape[1:])
            padded_batch[name] = np.concatenate([blobs[name][-remainder:], padding])
        yield padded_batch


def _Net_get_id_name(func, field):
    """Generic property that maps func to the layer names into an OrderedDict. Used for top_names and bottom_names.
    Parameters
        func: function id -> [id]
    ...
    def get_id_name(self):
        if not hasattr(self, field):
            ...
            res = OrderedDict([(self._layer_names[i], [id_to_name[j] for j in func(self, i)]) for i in range(len(self.layers))])
