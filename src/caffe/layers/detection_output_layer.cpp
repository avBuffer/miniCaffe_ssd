...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  variance_encoded_in_target_ = detection_output_param.variance_encoded_in_target();
  keep_top_k_ = detection_output_param.keep_top_k();
  confidence_threshold_ = detection_output_param.has_confidence_threshold() ? detection_output_param.confidence_threshold() : -FLT_MAX;
  ...
  const SaveOutputParameter& save_output_param = detection_output_param.save_output_param();
  ...
  if (save_output_param.has_label_map_file()) {
    ...
    if (label_map_file.empty()) {
      ...
      LabelMap label_map;
      CHECK(ReadProtoFromTextFile(label_map_file, &label_map)) << "Failed to read label map file: " << label_map_file;
      CHECK(MapLabelToName(label_map, true, &label_to_name_)) << "Failed to convert label to name.";
      CHECK(MapLabelToDisplayName(label_map, true, &label_to_display_name_)) << "Failed to convert label to display name.";
    }
  } else {
    need_save_ = false;
  }
  ...
  if (save_output_param.has_name_size_file()) {
    ...
    if (name_size_file.empty()) {
      ...
      // The file is in the following format: name height width ...
      string name;
      int height, width;
  ...
  if (visualize_) {
    ...
    data_transformer_.reset(new DataTransformer<Dtype>(this->layer_param_.transform_param(), this->phase_));
...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (need_save_) {
    ...
    if (name_count_ % num_test_image_ == 0) {
      ...
      if (output_format_ == "VOC") {
        ...
        for (map<int, string>::iterator it = label_to_name_.begin(); it != label_to_name_.end(); ++it) {
          if (it->first == background_label_id_) {
            continue;
          }
          std::ofstream outfile;
          boost::filesystem::path file(output_name_prefix_ + it->second + ".txt");
  ...
  if (bbox_preds_.num() != bottom[0]->num() || bbox_preds_.count(1) != bottom[0]->count(1)) {
    bbox_preds_.ReshapeLike(*(bottom[0]));
  }
  ...
  if (conf_permute_.num() != bottom[1]->num() || conf_permute_.count(1) != bottom[1]->count(1)) {
    conf_permute_.ReshapeLike(*(bottom[1]));
  }
  ...
  // Each row is a 7 dimension vector, which stores [image_id, label, confidence, xmin, ymin, xmax, ymax]
...
void DetectionOutputLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, 
  ...
  GetLocPredictions(loc_data, num, num_priors_, num_loc_classes_, share_location_, &all_loc_preds);

  // Retrieve all confidences.
  vector<map<int, vector<float> > > all_conf_scores;
  GetConfidenceScores(conf_data, num, num_priors_, num_classes_, &all_conf_scores);
  ...
  DecodeBBoxesAll(all_loc_preds, prior_bboxes, prior_variances, num, share_location_, num_loc_classes_, 
                  background_label_id_, code_type_, variance_encoded_in_target_, clip_bbox, &all_decode_bboxes);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin(); it != all_indices[i].end(); ++it) {
      ...
      if (need_save_) {
        CHECK(label_to_name_.find(label) != label_to_name_.end()) << "Cannot find label: " << label << " in the label map.";
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        if (need_save_) {
          ...
          OutputBBox(bbox, sizes_[name_count_], has_resize_, resize_param_, &out_bbox);
  ...
  if (visualize_) {
    ...
    VisualizeBBox(cv_imgs, top[0], visualize_threshold_, colors, label_to_display_name_, save_file_);
#endif  // USE_OPENCV
  }
}
