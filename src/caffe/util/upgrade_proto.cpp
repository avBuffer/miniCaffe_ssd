...
bool NetNeedsUpgrade(const NetParameter& net_param) {
  return NetNeedsV0ToV1Upgrade(net_param) || NetNeedsV1ToV2Upgrade(net_param) || NetNeedsDataUpgrade(net_param) ||
         NetNeedsInputUpgrade(net_param) || NetNeedsBatchNormUpgrade(net_param);
}
...
bool UpgradeNetAsNeeded(const string& param_file, NetParameter* param) {
  ...
  if (NetNeedsV0ToV1Upgrade(*param)) {
    ...
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated V0LayerParameter: " << param_file;
    NetParameter original_param(*param);
    
    if (!UpgradeV0Net(original_param, param)) {
      success = false;
      LOG(ERROR) << "Warning: had one or more problems upgrading V0NetParameter to NetParameter (see above); continuing anyway.";
    } else {
      LOG(INFO) << "Successfully upgraded file specified using deprecated V0LayerParameter";
    }

    LOG(WARNING) << "Note that future Caffe releases will not support V0NetParameter; use ./build/tools/upgrade_net_proto_text for " <<
        "prototxt and ./build/tools/upgrade_net_proto_binary for model weights upgrade this and any other net protos to the new format.";
  }

  // NetParameter uses old style data transformation fields; try to upgrade it.
  if (NetNeedsDataUpgrade(*param)) {
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated transformation parameters: " << param_file;
    UpgradeNetDataTransformation(param);
    LOG(INFO) << "Successfully upgraded file specified using deprecated data transformation parameters.";
    LOG(WARNING) << "Note that future Caffe releases will only support transform_param messages for transformation fields.";
  }

  if (NetNeedsV1ToV2Upgrade(*param)) {
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated V1LayerParameter: " << param_file;
    NetParameter original_param(*param);
    if (!UpgradeV1Net(original_param, param)) {
      success = false;
      LOG(ERROR) << "Warning: had one or more problems upgrading V1LayerParameter (see above); continuing anyway.";
    } else {
      LOG(INFO) << "Successfully upgraded file specified using deprecated V1LayerParameter";
    }
  }

  // NetParameter uses old style input fields; try to upgrade it.
  if (NetNeedsInputUpgrade(*param)) {
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated input fields: " << param_file;
    UpgradeNetInput(param);
    LOG(INFO) << "Successfully upgraded file specified using deprecated input fields.";
    LOG(WARNING) << "Note that future Caffe releases will only support input layers and not input fields.";
  }

  // NetParameter uses old style batch norm layers; try to upgrade it.
  if (NetNeedsBatchNormUpgrade(*param)) {
    LOG(INFO) << "Attempting to upgrade batch norm layers using deprecated params: " << param_file;
    UpgradeNetBatchNorm(param);
    LOG(INFO) << "Successfully upgraded batch norm layers using deprecated params.";
  }
  return success;
}
...
void ReadNetParamsFromTextFileOrDie(const string& param_file, NetParameter* param) {
  CHECK(ReadProtoFromTextFile(param_file, param)) << "Failed to parse NetParameter file: " << param_file;
  UpgradeNetAsNeeded(param_file, param);
}

void ReadNetParamsFromBinaryFileOrDie(const string& param_file, NetParameter* param) {
  CHECK(ReadProtoFromBinaryFile(param_file, param)) << "Failed to parse NetParameter file: " << param_file;
  UpgradeNetAsNeeded(param_file, param);
}
...
bool UpgradeV0Net(const NetParameter& v0_net_param_padding_layers, NetParameter* net_param) {
  ...
  for (int i = 0; i < v0_net_param.layers_size(); ++i) {
    is_fully_compatible &= UpgradeV0LayerParameter(v0_net_param.layers(i), net_param->add_layers());
  }
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    for (int j = 0; j < layer_connection.bottom_size(); ++j) {
      ...
      if (source_layer.layer().type() == "padding") {
        // This layer has a padding layer as input -- check that it is a conv layer or a pooling layer and takes only one input. 
        // Also check that the padding layer input has only one input and one output. Other cases have undefined behavior in Caffe.
        CHECK((layer_param.type() == "conv") || (layer_param.type() == "pool")) << 
            "Padding layer input to non-convolutional / non-pooling layer type " << layer_param.type();
        ...
        param_upgraded_pad->mutable_layers(layer_index)->mutable_layer()->set_pad(source_layer.layer().pad());
        param_upgraded_pad->mutable_layers(layer_index)->set_bottom(j, source_layer.bottom(0));
      }
    }
...
bool UpgradeV0LayerParameter(const V1LayerParameter& v0_layer_connection, V1LayerParameter* layer_param) {
  ...
  if (v0_layer_connection.has_layer()) {
    ...
    if (v0_layer_param.has_biasterm()) {
      if (type == "conv") {
        layer_param->mutable_convolution_param()->set_bias_term(v0_layer_param.biasterm());
      } else if (type == "innerproduct") {
        layer_param->mutable_inner_product_param()->set_bias_term(v0_layer_param.biasterm());
      } else {
    ...
    if (v0_layer_param.has_weight_filler()) {
      if (type == "conv") {
        ...
        layer_param->mutable_inner_product_param()->mutable_weight_filler()->CopyFrom(v0_layer_param.weight_filler());
      } else {
    ...
    if (v0_layer_param.has_bias_filler()) {
      if (type == "conv") {
        layer_param->mutable_convolution_param()->mutable_bias_filler()->CopyFrom(v0_layer_param.bias_filler());
      } else if (type == "innerproduct") {
        layer_param->mutable_inner_product_param()->mutable_bias_filler()->CopyFrom(v0_layer_param.bias_filler());
      } else {
    ...
    if (v0_layer_param.has_kernelsize()) {
      if (type == "conv") {
        layer_param->mutable_convolution_param()->add_kernel_size(v0_layer_param.kernelsize());
      } else if (type == "pool") {
        layer_param->mutable_pooling_param()->set_kernel_size(v0_layer_param.kernelsize());
      } else {
    ...
    if (v0_layer_param.has_group()) {
      if (type == "conv") {
        layer_param->mutable_convolution_param()->set_group(v0_layer_param.group());
      } else {
    ...
    if (v0_layer_param.has_stride()) {
      if (type == "conv") {
        layer_param->mutable_convolution_param()->add_stride(v0_layer_param.stride());
      } else if (type == "pool") {
        layer_param->mutable_pooling_param()->set_stride(v0_layer_param.stride());
      } else {
    ...
    if (v0_layer_param.has_dropout_ratio()) {
      if (type == "dropout") {
        layer_param->mutable_dropout_param()->set_dropout_ratio(v0_layer_param.dropout_ratio());
      } else {
    ...
    if (v0_layer_param.has_local_size()) {
      if (type == "lrn") {
        layer_param->mutable_lrn_param()->set_local_size(v0_layer_param.local_size());
      } else {
    ...
    if (v0_layer_param.has_source()) {
      if (type == "data") {
        ...
        layer_param->mutable_image_data_param()->set_source(v0_layer_param.source());
      } else if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_source(v0_layer_param.source());
      } else if (type == "infogain_loss") {
        layer_param->mutable_infogain_loss_param()->set_source(v0_layer_param.source());
      } else {
    ...
    if (v0_layer_param.has_meanfile()) {
      layer_param->mutable_transform_param()->set_mean_file(v0_layer_param.meanfile());
    }

    if (v0_layer_param.has_batchsize()) {
      if (type == "data") {
        layer_param->mutable_data_param()->set_batch_size(v0_layer_param.batchsize());
      } else if (type == "images") {
        layer_param->mutable_image_data_param()->set_batch_size(v0_layer_param.batchsize());
      } else if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_batch_size(v0_layer_param.batchsize());
      } else {
    ...
    if (v0_layer_param.has_cropsize()) {
      layer_param->mutable_transform_param()->set_crop_size(v0_layer_param.cropsize());
    }

    if (v0_layer_param.has_mirror()) {
      layer_param->mutable_transform_param()->set_mirror(v0_layer_param.mirror());
    }

    if (v0_layer_param.has_rand_skip()) {
      if (type == "data") {
        layer_param->mutable_data_param()->set_rand_skip(v0_layer_param.rand_skip());
      } else if (type == "images") {
        layer_param->mutable_image_data_param()->set_rand_skip(v0_layer_param.rand_skip());
      } else {
    ...
    if (v0_layer_param.has_shuffle_images()) {
      if (type == "images") {
        layer_param->mutable_image_data_param()->set_shuffle(v0_layer_param.shuffle_images());
      } else {
    ...
    if (v0_layer_param.has_new_height()) {
      if (type == "images") {
        layer_param->mutable_image_data_param()->set_new_height(v0_layer_param.new_height());
      } else {
    ...
    if (v0_layer_param.has_new_width()) {
      if (type == "images") {
        layer_param->mutable_image_data_param()->set_new_width(v0_layer_param.new_width());
      } else {
    ...
    if (v0_layer_param.has_concat_dim()) {
      if (type == "concat") {
        layer_param->mutable_concat_param()->set_concat_dim(v0_layer_param.concat_dim());
      } else {
    ...
    if (v0_layer_param.has_det_fg_threshold()) {
      if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_fg_threshold(v0_layer_param.det_fg_threshold());
      } else {
        LOG(ERROR) << "Unknown parameter det_fg_threshold for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_det_bg_threshold()) {
      if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_bg_threshold(v0_layer_param.det_bg_threshold());
      } else {
        LOG(ERROR) << "Unknown parameter det_bg_threshold for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_det_fg_fraction()) {
      if (type == "window_data") {
        ...
        LOG(ERROR) << "Unknown parameter det_fg_fraction for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_det_context_pad()) {
      if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_context_pad(v0_layer_param.det_context_pad());
      } else {
        LOG(ERROR) << "Unknown parameter det_context_pad for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_det_crop_mode()) {
      if (type == "window_data") {
        layer_param->mutable_window_data_param()->set_crop_mode(v0_layer_param.det_crop_mode());
      } else {
...
#define CONVERT_LAYER_TRANSFORM_PARAM(TYPE, Name, param_name) \
  do { \
    if (net_param->layers(i).type() == V1LayerParameter_LayerType_##TYPE) { \
      Name##Parameter* layer_param = net_param->mutable_layers(i)->mutable_##param_name##_param(); \
      TransformationParameter* transform_param = net_param->mutable_layers(i)->mutable_transform_param(); \
...
bool UpgradeV1Net(const NetParameter& v1_net_param, NetParameter* net_param) {
  if (v1_net_param.layer_size() > 0) {
    LOG(FATAL) << "Refusing to upgrade inconsistent NetParameter input; the definition includes both 'layer' and 'layers' fields. " <<
        "The current format defines 'layer' fields with string type like layer { type: 'Layer' ... } and not layers { type: LAYER ... }. " <<
  ...
  for (int i = 0; i < v1_net_param.layers_size(); ++i) {
    if (!UpgradeV1LayerParameter(v1_net_param.layers(i), net_param->add_layer())) {
...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param, LayerParameter* layer_param) {
  ...
  for (int i = 0; i < v1_layer_param.blob_share_mode_size(); ++i) {
    ...
    switch (v1_layer_param.blob_share_mode(i)) {
      ...
      case V1LayerParameter_DimCheckMode_PERMISSIVE:
        ...
        break;
      default:
        LOG(FATAL) << "Unknown blob_share_mode: " << v1_layer_param.blob_share_mode(i);
        break;
    }
  ...
  for (int i = 0; i < v1_layer_param.weight_decay_size(); ++i) {
    while (layer_param->param_size() <= i) { layer_param->add_param(); }
    layer_param->mutable_param(i)->set_decay_mult(v1_layer_param.weight_decay(i));
  }
  ...
  if (v1_layer_param.has_accuracy_param()) {
    layer_param->mutable_accuracy_param()->CopyFrom(v1_layer_param.accuracy_param());
  }

  if (v1_layer_param.has_argmax_param()) {
    layer_param->mutable_argmax_param()->CopyFrom(v1_layer_param.argmax_param());
  }

  if (v1_layer_param.has_concat_param()) {
    layer_param->mutable_concat_param()->CopyFrom(v1_layer_param.concat_param());
  }

  if (v1_layer_param.has_contrastive_loss_param()) {
    layer_param->mutable_contrastive_loss_param()->CopyFrom(v1_layer_param.contrastive_loss_param());
  }

  if (v1_layer_param.has_convolution_param()) {
    layer_param->mutable_convolution_param()->CopyFrom(v1_layer_param.convolution_param());
  }
  ...
  if (v1_layer_param.has_dropout_param()) {
    layer_param->mutable_dropout_param()->CopyFrom(v1_layer_param.dropout_param());
  }

  if (v1_layer_param.has_dummy_data_param()) {
    layer_param->mutable_dummy_data_param()->CopyFrom(v1_layer_param.dummy_data_param());
  }

  if (v1_layer_param.has_eltwise_param()) {
    layer_param->mutable_eltwise_param()->CopyFrom(v1_layer_param.eltwise_param());
  }
  ...
  if (v1_layer_param.has_hinge_loss_param()) {
    layer_param->mutable_hinge_loss_param()->CopyFrom(v1_layer_param.hinge_loss_param());
  }

  if (v1_layer_param.has_image_data_param()) {
    layer_param->mutable_image_data_param()->CopyFrom(v1_layer_param.image_data_param());
  }

  if (v1_layer_param.has_infogain_loss_param()) {
    layer_param->mutable_infogain_loss_param()->CopyFrom(v1_layer_param.infogain_loss_param());
  }

  if (v1_layer_param.has_inner_product_param()) {
    layer_param->mutable_inner_product_param()->CopyFrom(v1_layer_param.inner_product_param());
  }
  ...
  if (v1_layer_param.has_memory_data_param()) {
    layer_param->mutable_memory_data_param()->CopyFrom(v1_layer_param.memory_data_param());
  }
  ...
  if (v1_layer_param.has_pooling_param()) {
    layer_param->mutable_pooling_param()->CopyFrom(v1_layer_param.pooling_param());
  }
  ...
  if (v1_layer_param.has_sigmoid_param()) {
    layer_param->mutable_sigmoid_param()->CopyFrom(v1_layer_param.sigmoid_param());
  }

  if (v1_layer_param.has_softmax_param()) {
    layer_param->mutable_softmax_param()->CopyFrom(v1_layer_param.softmax_param());
  }
  ...
  if (v1_layer_param.has_threshold_param()) {
    layer_param->mutable_threshold_param()->CopyFrom(v1_layer_param.threshold_param());
  }

  if (v1_layer_param.has_window_data_param()) {
    layer_param->mutable_window_data_param()->CopyFrom(v1_layer_param.window_data_param());
  }

  if (v1_layer_param.has_transform_param()) {
    layer_param->mutable_transform_param()->CopyFrom(v1_layer_param.transform_param());
  }
...
bool NetNeedsInputUpgrade(const NetParameter& net_param) { return net_param.input_size() > 0; }

void UpgradeNetInput(NetParameter* net_param) {
  // Collect inputs and convert to Input layer definitions. If the NetParameter holds an input alone, without shape/dim, 
  // then it's a legacy caffemodel and simply stripping the input field is enough.
...
bool UpgradeSolverType(SolverParameter* solver_param) {
  CHECK(!solver_param->has_solver_type() || !solver_param->has_type()) << 
      "Failed to upgrade solver: old solver_type field (enum) and new type field (string) cannot be both specified in solver proto text.";
  if (solver_param->has_solver_type()) {
    string type;
...
bool UpgradeSolverAsNeeded(const string& param_file, SolverParameter* param) {
  ...
  if (SolverNeedsTypeUpgrade(*param)) {
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated 'solver_type' field (enum)': " << param_file;
    if (!UpgradeSolverType(param)) {
      success = false;
      LOG(ERROR) << "Warning: had one or more problems upgrading SolverType (see above).";
    } else {
      LOG(INFO) << "Successfully upgraded file specified using deprecated 'solver_type' field (enum) to 'type' field (string).";
      LOG(WARNING) << "Note that future Caffe releases will only support 'type' field (string) for a solver's type.";
    }
  }
  return success;
}
...
void ReadSolverParamsFromTextFileOrDie(const string& param_file, SolverParameter* param) {
  CHECK(ReadProtoFromTextFile(param_file, param)) << "Failed to parse SolverParameter file: " << param_file;
