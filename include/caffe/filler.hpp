// Fillers are random number generators that fills a blob using the specified algorithm. 
// The expectation is that they are only going to be used during initialization time.
...
/** @brief Fills a Blob with values @f$ x \in [0, 1] @f$ such that @f$ \forall i \sum_j x_{ij} = 1 @f$. */
template <typename Dtype>
class PositiveUnitballFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    ...
    // We expect the filler to not be called very frequently, so we will just use a simple implementation
...
You can upsample a feature map with shape of (B, C, H, W) by any integer factor using the following proto.
\code
layer {
...
Please use this by replacing `{{}}` with your values. By specifying `num_output: {{C}} group: {{C}}`, 
it behaves as channel-wise convolution. The filter shape of this deconvolution layer will be
(C, 1, K, K) where K is `kernel_size`, and this filler will set a (K, K)
interpolation kernel for every channel of the filter identically. The resulting
shape of the top feature map will be (B, C, factor * H, factor * W).
Note that the learning rate and the weight decay are set to 0 in order to keep coefficient values of bilinear
