...
.. note::
    ...
    it requires graphviz and other prerequisites outside the scope of the Caffe."""
...
def get_layer_label(layer, rankdir):
    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        # Outer double quotes needed or else colon characters don't parse properly
        node_label = '"%s%s(%s)%skernel size: %d%sstride: %d%spad: %d"' % \
                     (layer.name, separator, layer.type, separator,
                      ...
                      separator, layer.convolution_param.stride[0] if len(layer.convolution_param.stride._values) else 1,
                      separator, layer.convolution_param.pad[0] if len(layer.convolution_param.pad._values) else 0)
    
    elif layer.type == 'Pooling':
        pooling_types_dict = get_pooling_types_dict()
        node_label = '"%s%s(%s %s)%skernel size: %d%sstride: %d%spad: %d"' % \
                     (layer.name, separator, pooling_types_dict[layer.pooling_param.pool],
                      layer.type, separator, layer.pooling_param.kernel_size,
                      separator, layer.pooling_param.stride, separator, layer.pooling_param.pad)
    else:
...
def get_pydot_graph(caffe_net, rankdir, label_edges=True, phase=None):
    ...
    Parameters
        ...
        phase : {caffe_pb2.Phase.TRAIN, caffe_pb2.Phase.TEST, None} optional
                Include layers from this network phase.  If None, include all layers. (the default is None)
    ...
    for layer in caffe_net.layer:
        if phase is not None:
          included = False
          if len(layer.include) == 0: included = True
    ...
    for node in pydot_nodes.values(): pydot_graph.add_node(node)
...
def draw_net(caffe_net, rankdir, ext='png', phase=None):
    ...
    Parameters
        ...
        ext : string, optional, The image extension (the default is 'png').
        phase : {caffe_pb2.Phase.TRAIN, caffe_pb2.Phase.TEST, None} optional
                Include layers from this network phase.  If None, include all layers. (the default is None)
    Returns
...
def draw_net_to_file(caffe_net, filename, rankdir='LR', phase=None):
    ...
    Use '.raw' to output raw text that you can manually feed to graphviz to draw graphs.
    Parameters
        caffe_net : a caffe.proto.caffe_pb2.NetParameter protocol buffer.
