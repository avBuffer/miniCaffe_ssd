...
template <typename Dtype>
inline Dtype tanh(Dtype x) { return 2. * sigmoid(2. * x) - 1.; }


template <typename Dtype>
void LSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype f = (*cont == 0) ? 0 : (*cont * sigmoid(X[1 * hidden_dim_ + d]));
...
template <typename Dtype>
void LSTMUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype c_term_diff = C_diff[d] + H_diff[d] * o * (1 - tanh_c * tanh_c);
