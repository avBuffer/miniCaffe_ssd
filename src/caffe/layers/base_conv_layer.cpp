...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if (conv_param.has_kernel_h() || conv_param.has_kernel_w()) {
    ...
    CHECK(num_kernel_dims == 1 || num_kernel_dims == num_spatial_axes_) <<
        "kernel_size must be specified once, or once per spatial dimension (kernel_size specified " << num_kernel_dims << 
  ...
  if (conv_param.has_stride_h() || conv_param.has_stride_w()) {
    ...
    CHECK(num_stride_dims == 0 || num_stride_dims == 1 || num_stride_dims == num_spatial_axes_) <<
        "stride must be specified once, or once per spatial dimension (stride specified " << num_stride_dims << 
        " times; " << num_spatial_axes_ << " spatial dims).";    
  ...
  if (conv_param.has_pad_h() || conv_param.has_pad_w()) {
    ...
    CHECK(num_pad_dims == 0 || num_pad_dims == 1 || num_pad_dims == num_spatial_axes_) <<
        "pad must be specified once, or once per spatial dimension (pad specified " << num_pad_dims << 
        " times; " << num_spatial_axes_ << " spatial dims).";    
  ...
  CHECK(num_dilation_dims == 0 || num_dilation_dims == 1 || num_dilation_dims == num_spatial_axes_) <<
      "dilation must be specified once, or once per spatial dimension (dilation specified " << num_dilation_dims << 
      " times; " << num_spatial_axes_ << " spatial dims).";
  ...
  // Special case: im2col is the identity for 1x1 convolution with stride 1 and no padding, 
  ...
  if (this->blobs_.size() > 0) {
    ...
    // Initialize and fill the weights: output channels x input channels per-group x kernel height x kernel width
...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int bottom_id = 1; bottom_id < bottom.size(); ++bottom_id) {
    CHECK(bottom[0]->shape() == bottom[bottom_id]->shape()) << "All inputs must have the same shape.";
  }
  ...
  // The im2col result buffer will only hold one image at a time to avoid overly large memory usage. 
  // In the special case of 1x1 convolution it goes lazily unused to save memory.
...
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output, const Dtype* weights, Dtype* input) {
  ...
  for (int g = 0; g < group_; ++g) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_, conv_out_spatial_dim_, conv_out_channels_ / group_,
...
void BaseConvolutionLayer<Dtype>::weight_cpu_gemm(const Dtype* input, const Dtype* output, Dtype* weights) {
  ...
  for (int g = 0; g < group_; ++g) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_, kernel_dim_, conv_out_spatial_dim_,
