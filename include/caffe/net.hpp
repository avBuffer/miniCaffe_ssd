...
class Net {
 public:
  ...
  const vector<Blob<Dtype>*>& ForwardPrefilled(Dtype* loss = NULL) {
    LOG_EVERY_N(WARNING, 1000) << "DEPRECATED: ForwardPrefilled() will be removed in a future version. Use Forward().";
    return Forward(loss);
  }

  /* The From and To variants of Forward and Backward operate on the (topological) ordering by which the net is specified.
   * For general DAG networks, note that (1) computing from one layer to another might entail extra computation on unrelated branches, 
   * and (2) computation starting in the middle may be incorrect if all of the layers of a fan-in are not included. */
  ...
  /* @brief Zeroes out the diffs of all net parameters. Should be run before Backward. */
  void ClearParamDiffs();

  /* The network backward should take no input and output, since it solely computes the gradient w.r.t the parameters,
   * and the data has already been provided during the forward pass. */
  void Backward();
  ...
  /* @brief For an already initialized net, implicitly copies (i.e., using no additional memory) 
            the pre-trained layers from another Net. */
  void ShareTrainedLayersWith(const Net* other);
  
  // For an already initialized net, CopyTrainedLayersFrom() copies the already
  // trained layers from another net parameter instance.
  /* @brief For an already initialized net, copies the pre-trained layers from another Net. */
  ...
  inline const vector<string>& param_display_names() const { return param_display_names_; }
  ...
  // Helpers for Init.
  /* @brief Remove layers that the user specified should be excluded given the current phase, level, and stage. */
 ...
 protected:
  ...
  /* The mapping from params_ -> learnable_params_: we have learnable_param_ids_.size() == params_.size(),
