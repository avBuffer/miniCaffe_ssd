...
PYTHON_INCLUDE := /usr/include/python3.8 /usr/lib/python3.8/dist-packages/numpy/core/include
...
INCLUDE_DIRS := $(PYTHON_INCLUDE) /usr/local/include /usr/include/opencv4 
...
ALL_BUILD_DIRS := $(sort $(BUILD_DIR) $(addprefix $(BUILD_DIR)/, $(SRC_DIRS)) \
	$(LIB_BUILD_DIR) $(PY_PROTO_BUILD_DIR) $(LINT_OUTPUT_DIR) $(PROTO_BUILD_INCLUDE_DIR))
...
##############################
.PHONY: all lib clean linecount lint lintclean tools examples $(DIST_ALIASES) \
	py py$(PROJECT) proto superclean supercleanlist supercleanfiles warn everything
...
linecount:
	cloc --read-lang-def=$(PROJECT).cloc src/$(PROJECT) include/$(PROJECT) tools examples python 
...
$(PY_PROTO_BUILD_DIR)/%_pb2.py : $(PROTO_SRC_DIR)/%.proto $(PY_PROTO_INIT) | $(PY_PROTO_BUILD_DIR)
	@ echo PROTOC \(python\) $<
