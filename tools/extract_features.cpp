...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  if (argc < num_required_args) {
    LOG(ERROR) << "This program takes in a trained network and an input data layer, and then"
                  ...
                  " The names cannot contain white space characters and the number of blobs and datasets must be equal.";
    return 1;
  }
  ...
  // Expected prototxt contains at least one data layer such as the layer data_layer_name
  // and one feature blob such as the fc7 top blob to extract features.
    /*
   layers {
     name: "data_layer_name"
     type: DATA
     data_param {
       source: "/path/to/your/images/to/extract/feature/images_lmdb"
  ...
  for (size_t i = 0; i < num_features; i++) {
    CHECK(feature_extraction_net->has_blob(blob_names[i])) << "Unknown feature blob name " << blob_names[i] << 
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        if (image_indices[i] % 1000 == 0) {
          ...
          LOG(ERROR)<< "Extracted features of " << image_indices[i] << " query images for feature blob " << blob_names[i];
        }
      }  // for (int n = 0; n < batch_size; ++n)
    }  // for (int i = 0; i < num_features; ++i)
  }  // for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index)
