...
class SGDSolver : public Solver<Dtype> {
 ...
 protected:
  ...
  // history maintains the historical momentum data. update maintains update related data and is not needed in snapshots.
  // temp maintains other information that might be needed in computation of gradients/updates and is not needed in snapshots
...
class RMSPropSolver : public SGDSolver<Dtype> {
 ...
 protected:
  virtual void ComputeUpdateValue(int param_id, Dtype rate);
  void constructor_sanity_check() {
    CHECK_EQ(0, this->param_.momentum()) << "Momentum cannot be used with RMSProp.";
    CHECK_GE(this->param_.rms_decay(), 0) << "rms_decay should lie between 0 and 1.";
    CHECK_LT(this->param_.rms_decay(), 1) << "rms_decay should lie between 0 and 1.";
  }
...
/* @brief AdamSolver, an algorithm for first-order gradient-based optimization of stochastic objective functions,
 *        based on adaptive estimates of lower-order moments. Described in [1].
