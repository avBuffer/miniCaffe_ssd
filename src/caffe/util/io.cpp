...
bool ReadProtoFromBinaryFile(const char* filename, Message* proto) {
  ...
  coded_input->SetTotalBytesLimit(kProtoReadBytesLimit, 536870912); // 512 * 1024 * 1024 = 536870912 (512M)
...
bool ReadRichImageToAnnotatedDatum(const string& filename, const string& labelfile, const int height, const int width,
                                   const int min_dim, const int max_dim, const bool is_color, const string& encoding, 
  ...
  // Read image to datum.
  bool status = ReadImageToDatum(filename, -1, height, width, min_dim, max_dim, is_color, encoding,
  ...
  switch (type) {
    case AnnotatedDatum_AnnotationType_BBOX:
      ...
      if (labeltype == "xml") {
        return ReadXMLToAnnotatedDatum(labelfile, ori_height, ori_width, name_to_label, anno_datum);
      } else if (labeltype == "json") {
        return ReadJSONToAnnotatedDatum(labelfile, ori_height, ori_width, name_to_label, anno_datum);
...
bool ReadXMLToAnnotatedDatum(const string& labelfile, const int img_height, const int img_width, 
                             const std::map<string, int>& name_to_label, AnnotatedDatum* anno_datum) {
  ptree pt;
  ...
  LOG_IF(WARNING, height != img_height) << labelfile << " inconsistent image height.";
  LOG_IF(WARNING, width != img_width) << labelfile << " inconsistent image width.";
  CHECK(width != 0 && height != 0) << labelfile << " no valid image width/height.";
  int instance_id = 0;
    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          CHECK_NOTNULL(anno);
          LOG_IF(WARNING, xmin > width) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, ymin > height) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, xmax > width) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, ymax > height) << labelfile << " bounding box exceeds image boundary.";

          LOG_IF(WARNING, xmin < 0) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, ymin < 0) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, xmax < 0) << labelfile << " bounding box exceeds image boundary.";
          LOG_IF(WARNING, ymax < 0) << labelfile << " bounding box exceeds image boundary.";
...
bool ReadJSONToAnnotatedDatum(const string& labelfile, const int img_height, const int img_width, 
                              const std::map<string, int>& name_to_label, AnnotatedDatum* anno_datum) {
  ptree pt;
...
bool ReadLabelFileToLabelMap(const string& filename, bool include_background,
  ...
  // The order of the fields are: name [label] [display_name] ...
  int field_size = -1;
  int label = 0;
  ...
  while (std::getline(file, line)) {
    ...
    if (field_size == -1) {
      ...
      CHECK_EQ(field_size, fields.size()) << "Inconsistent number of fields per line.";
    }
...
bool MapLabelToDisplayName(const LabelMap& map, const bool strict_check,
  ...
  for (int i = 0; i < map.item_size(); ++i) {
    ...
    const int label = map.item(i).label();
    if (strict_check) {
      if (!label_to_display_name->insert(std::make_pair(label, display_name)).second) {
        LOG(FATAL) << "There are many duplicates of label: " << label;
        return false;
      }
    } else {
