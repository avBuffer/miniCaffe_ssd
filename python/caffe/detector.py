...
This implementation follows ideas in Ross Girshick, Jeff Donahue, Trevor Darrell, Jitendra Malik.
...
class Detector(caffe.Net):
    ...
    Parameters
        ...
        context_pad : amount of surrounding context to take s.t. a `context_pad` sized border of pixels 
                      in the network input image is context, as in R-CNN feature extraction."""
    def __init__(self, model_file, pretrained_file, mean=None, input_scale=None, 
                 raw_scale=None, channel_swap=None, context_pad=None):
    ...
    def crop(self, im, window):
        ...
        if self.context_pad:
            ...
            # collect with context padding and place in input with mean padding
    ...
    def configure_crop(self, context_pad):
        ...
        self.crop_dims = np.array(self.blobs[in_].data.shape[1:])[inv_tpose] #.transpose(inv_tpose)
        ...
        if self.context_pad:
            ...
            if mean is not None:
                ...
                if channel_order is not None:
                    channel_order_inverse = [channel_order.index(i) for i in range(crop_mean.shape[2])]
