...
template <typename Dtype>
AnnotatedDataLayer<Dtype>::~AnnotatedDataLayer() { this->StopInternalThread(); }


template <typename Dtype>
void AnnotatedDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, 
  ...
  if (this->output_labels_) {
    ...
    if (has_anno_type_) {
      ...
      if (anno_data_param.has_anno_type()) {
        // If anno_type is provided in AnnotatedDataParameter, replace the type stored in each individual AnnotatedDatum.
      ...
      if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
        ...
        // Note: Refer to caffe.proto for details about group_label and instance_id.
        ...
        // BasePrefetchingDataLayer<Dtype>::LayerSetUp() requires to call cpu_data and gpu_data for consistent prefetch thread. 
        // Thus we make sure there is at least one bbox.
...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  // Reshape according to the first anno_datum of each batch on single input batches allows for inputs of varying dimension.
  const int batch_size = this->layer_param_.data_param().batch_size();
  const AnnotatedDataParameter& anno_data_param = this->layer_param_.annotated_data_param();
  const TransformationParameter& transform_param = this->layer_param_.transform_param();
  AnnotatedDatum& anno_datum = *(reader_.full().peek());
