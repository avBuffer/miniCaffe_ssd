...
void Solver<Dtype>::SetActionFunction(ActionCallback func) { action_request_function_ = func; }
...
template <typename Dtype>
Solver<Dtype>::Solver(const string& param_file, const Solver* root_solver) : net_(), callbacks_(), root_solver_(root_solver), requested_early_exit_(false) {
...
void Solver<Dtype>::CheckType(SolverParameter* param) {
  ...
  if (param->has_type()) {
    CHECK_EQ(param->type(), this->type()) << "Solver type must agree with instantiated solver class.";
  } else {
...
void Solver<Dtype>::InitTrainNet() {
  const int num_train_nets = param_.has_net() + param_.has_net_param() + param_.has_train_net() + param_.has_train_net_param();
  const string field_names = "net, net_param, train_net, train_net_param";
  CHECK_GE(num_train_nets, 1) << "SolverParameter must specify a train net using one of these fields: " << field_names;
  CHECK_LE(num_train_nets, 1) << "SolverParameter must not contain more than one of these fields specifying a train_net: " << field_names;
  NetParameter net_param;
  
  if (param_.has_train_net_param()) {
    LOG_IF(INFO, Caffe::root_solver()) << "Creating training net specified in train_net_param.";
  ...
  // Set the correct NetState.  We start with the solver defaults (lowest precedence); 
  // then, merge in any NetState specified by the net_param itself;
  // finally, merge in any NetState specified by the train_state (highest precedence).
  NetState net_state;
...
void Solver<Dtype>::InitTestNets() {
  ...
  CHECK_LE(num_generic_nets, 1) << "Both net_param and net_file may not be specified.";
  const int num_test_net_params = param_.test_net_param_size();
  const int num_test_net_files = param_.test_net_size();
  const int num_test_nets = num_test_net_params + num_test_net_files;
  if (num_generic_nets) {
      CHECK_GE(param_.test_iter_size(), num_test_nets) << "test_iter must be specified for each test network.";
  } else {
      CHECK_EQ(param_.test_iter_size(), num_test_nets) << "test_iter must be specified for each test network.";
  }

  // If we have a generic net (specified by net or net_param, rather than test_net or test_net_param), 
  // we may have an unlimited number of actual test networks -- the actual number is given by the number of remaining
  // test_iters after any test nets specified by test_net_param and/or test_net are evaluated.
  const int num_generic_net_instances = param_.test_iter_size() - num_test_nets;
  const int num_test_net_instances = num_test_nets + num_generic_net_instances;
  if (param_.test_state_size()) {
    CHECK_EQ(param_.test_state_size(), num_test_net_instances) << "test_state must be unspecified or specified once per test net.";
  }
  ...
  for (int i = 0; i < num_test_net_instances; ++i) {
    // Set the correct NetState. We start with the solver defaults (lowest precedence); 
    // then, merge in any NetState specified by the net_param itself;
    // finally, merge in any NetState specified by the test_state (highest precedence).
    NetState net_state;
...
void Solver<Dtype>::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
    ...
    if (param_.test_interval() && iter_ % param_.test_interval() == 0 && (iter_ > 0 || param_.test_initialization()) && 
	    Caffe::root_solver()) {
      TestAll();
    ...
    if (display) {
      ...
      for (int j = 0; j < result.size(); ++j) {
        ...
        for (int k = 0; k < result[j]->count(); ++k) {
          ...
          LOG_IF(INFO, Caffe::root_solver()) << "    Train net output #" << score_index++ << ": " << 
    ...
    ApplyUpdate();
    // Increment the internal iter_ counter -- its value should always indicate the number of times the weights have been updated.
    ++iter_;
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  // For a network that is trained by the solver, no bottom or top vecs should be given, and we will just provide dummy vecs.
...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    for (map<int, int>::const_iterator it = num_pos.begin(); it != num_pos.end(); ++it) {
      ...
      ComputeAP(label_true_pos, label_num_pos, label_false_pos, param_.ap_version(), &prec, &rec, &(APs[label]));
      mAP += APs[label];
...
void Solver<Dtype>::CheckSnapshotWritePermissions() {
  if (Caffe::root_solver() && param_.snapshot()) {
    CHECK(param_.has_snapshot_prefix()) << "In solver params, snapshot is specified but snapshot_prefix is not";
    ...
    if (probe_ofs.good()) {
      ...
      LOG(FATAL) << "Cannot write to snapshot prefix '" << param_.snapshot_prefix() << "'.  Make sure that the directory exists and is writable.";
    }
  }
}
