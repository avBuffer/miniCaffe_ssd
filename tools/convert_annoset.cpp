...
// Usage: convert_annoset [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME

// where ROOTFOLDER is the root folder that holds all the images and annotations, 
...
DEFINE_string(anno_type, "classification", "The type of annotation {classification, detection}.");
...
int main(int argc, char** argv) {
  ...
  gflags::SetUsageMessage("Convert a set of images and annotations to the lmdb format used as input for Caffe.\n"
                          "Usage: convert_annoset [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME\n");
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (anno_type == "classification") {
      ...
      status = ReadImageToDatum(filename, label, resize_height, resize_width, min_dim, max_dim, is_color, enc, datum);
    } else if (anno_type == "detection") {
      labelname = root_folder + boost::get<std::string>(lines[line_id].second);
      status = ReadRichImageToAnnotatedDatum(filename, labelname, resize_height, resize_width, min_dim, max_dim, is_color,
                                             enc, type, label_type, name_to_label, &anno_datum);
