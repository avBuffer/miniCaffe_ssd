...
/* @brief A generalization of SoftmaxWithLossLayer that takes an "information gain" (infogain) matrix
 *        specifying the "value" of all label pairs.
 * Equivalent to the SoftmaxWithLossLayer if the infogain matrix is the identity.
 *
 * @param bottom input Blob vector (length 2-3)
 *   -# @f$ (N \times C \times H \times W) @f$ the predictions @f$ x @f$, a Blob with values in
 *      @f$ [-\infty, +\infty] @f$ indicating the predicted score for each of
 *      the @f$ K = CHW @f$ classes. This layer maps these scores to a
 *      probability distribution over classes using the softmax function
 *      @f$ \hat{p}_{nk} = \exp(x_{nk}) / \left[\sum_{k'} \exp(x_{nk'})\right] @f$ (see SoftmaxLayer).
 ...
 *   -# @f$ (1 \times 1 \times K \times K) @f$ (\b optional) the infogain matrix @f$ H @f$.  This must be provided as
 *      the third bottom blob input if not provided as the infogain_mat in the InfogainLossParameter. 
 *      If @f$ H = I @f$, this layer is equivalent to the SoftmaxWithLossLayer.
...
class InfogainLossLayer : public LossLayer<Dtype> {
 public:
  explicit InfogainLossLayer(const LayerParameter& param) : LossLayer<Dtype>(param), infogain_() {}
  ...
  // (Otherwise the infogain matrix is loaded from a file specified by LayerParameter.)
 ...
 protected:
   ...
   *      @f$\ell_i@f$ in the overall Net loss @f$ E = \lambda_i \ell_i + \mbox{other loss terms}@f$; hence
   *      @f$ \frac{\partial E}{\partial \ell_i} = \lambda_i @f$.
   *      (*Assuming that this top Blob is not used as a bottom (input) by any other layer of the Net.)
   * @param propagate_down see Layer::Backward. propagate_down[1] must be false as we can't compute gradients with
   *      respect to the labels (similarly for propagate_down[2] and the infogain matrix, if provided as bottom[2])
   * @param bottom input Blob vector (length 2-3)
   *   -# @f$ (N \times C \times H \times W) @f$ the predictions @f$ x @f$; Backward computes diff @f$ \frac{\partial E}{\partial x} @f$
   *   -# @f$ (N \times 1 \times 1 \times 1) @f$ the labels -- ignored as we can't compute their error gradients
   *   -# @f$ (1 \times 1 \times K \times K) @f$ (\b optional) the information gain matrix -- ignored as its error gradient computation is not implemented. */
